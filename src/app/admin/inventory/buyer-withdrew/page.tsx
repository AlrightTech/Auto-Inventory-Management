...
import { BuyerWithdrewVehicleTable } from '@/components/inventory/BuyerWithdrewVehicleTable';
import { Search, Filter, RotateCcw, Download, FileText, Package, AlertTriangle } from 'lucide-react';
...
export default function BuyerWithdrewPage() {
  ...
  const handleExportCSV = () => {
    ...
    try {
      ...
      link.href = url;
      link.download = `buyer-withdrew-${new Date().toISOString().split('T')[0]}.csv`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      window.URL.revokeObjectURL(url);
      
      toast.success(`Exported ${filteredVehicles.length} vehicles to CSV`);
    } catch (error) {
  ...
  const handleExportPDF = () => {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Buyer Withdrew');

      XLSX.writeFile(wb, `buyer-withdrew-${new Date().toISOString().split('T')[0]}.xlsx`);
      
      toast.success(`Exported ${filteredVehicles.length} vehicles. Open the Excel file and use "Save As PDF" to create a PDF.`);
    } catch (error) {
  ...
  // Load vehicles with status 'Withdrew'
  useEffect(() => {
          ...
          <Card className="dashboard-card neon-glow instrument-cluster">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium" style={{ color: 'var(--subtext)' }}>Total Withdrew</p>
        ...
        >
          <Card className="dashboard-card neon-glow instrument-cluster">
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium" style={{ color: 'var(--subtext)' }}>Missing Locations</p>
                  <p className="text-3xl font-bold" style={{ color: 'var(--text)' }}>
