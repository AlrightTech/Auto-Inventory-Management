...
export function Header({ user, onMenuClick }: HeaderProps) {
  ...
  return (
    <header className="glass-card-strong border-b border-slate-700/50 px-6 py-4">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent className="w-80 glass-card-strong border-slate-700" align="end" forceMount>
                ...
                {notifications.slice(0, 10).map(n => (
                  <DropdownMenuItem key={n.id} className="text-slate-200 hover:bg-slate-700/50">
                    <div className="flex items-start gap-2 w-full">
                      <div className="mt-1">
                        <span className={`inline-block w-2 h-2 rounded-full ${n.read ? 'bg-slate-500' : 'bg-blue-500'}`} />
                      </div>
                      ...
                      {!n.read && (
                        <button onClick={() => markAsRead(n.id)} className="text-xs text-blue-400 hover:text-blue-300">Mark read</button>
                      )}
                    </div>
                  </DropdownMenuItem>
                ))}
          ...
          {user && (
            ...
            >
              <DropdownMenu>
                ...
                <DropdownMenuContent className="w-56 glass-card-strong border-slate-700" align="end" forceMount>
